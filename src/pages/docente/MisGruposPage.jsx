          ...
          {grupos.map(g => (
            <div key={g.id} className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm hover:shadow-md transition">
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-1">
                    ...
                    <p className="text-xs text-indigo-600 font-medium truncate">{g.reunion?.nombre} · {g.reunion?.hora_inicio}–{g.reunion?.hora_fin}</p>
                  </div>
                </div>
              </div>
              ...
              {g.ayudantes_checklist && (
                <div className="mt-3 flex items-center gap-2 text-xs bg-emerald-50 text-emerald-700 px-3 py-2 rounded-xl">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 shrink-0" />
                  Checklist habilitado para ayudantes
                </div>
              )}
            </div>
          ))}
