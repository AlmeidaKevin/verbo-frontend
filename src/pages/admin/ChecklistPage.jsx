...
const ChecklistPage = () => {
  ...
  const cargarRegistro = async (nuevo, registroId = null) => {
    ...
    try {
      let registroData;
      if (registroId) {
        const { data } = await api.post('/asistencias/registro', { reunion_id: reunionSel, grupo_id: grupoSel, nuevo: false });
        registroData = data;
      } else {
        const { data } = await api.post('/asistencias/registro', { reunion_id: reunionSel, grupo_id: grupoSel, nuevo });
        registroData = data;
      }
  ...
  if (modoSelector) {
    ...
    return (
      <div className="max-w-lg mx-auto">
        ...
        <p className="text-gray-500 text-sm mb-6">{reunionInfo?.nombre} · {grupoInfo?.nombre}</p>
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-5 flex gap-3">
          ...
          <p className="text-sm text-amber-700">Ya hay <strong>{registrosHoy.length}</strong> lista(s) registradas hoy para este grupo. ¿Deseas continuar con una existente o crear una nueva?</p>
        </div>
        ...
        <button onClick={() => setModoSelector(false)} className="w-full mt-2 py-3 border border-gray-300 rounded-xl text-sm text-gray-500 hover:bg-gray-50 transition">Cancelar</button>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4">
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input ref={busquedaRef} value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="Buscar niño..."
          ...
          <button onClick={() => setModalAgregar(true)} className="flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white px-4 py-2.5 rounded-xl text-sm font-medium transition">
              ...
              <div key={a.id} className="flex items-center justify-between px-4 py-3">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => setModalComentario({ id: a.id, nombre: a.nino?.nombre_completo, comentario: a.comentario || '', tarde: a.llego_tarde || false })}
                    className="p-2 text-gray-400 hover:text-primary-600 transition" title="Editar comentario">
                    <FiMessageSquare size={16} />
                  </button>
                  <button onClick={() => desmarcarAsistencia(a.id, a.nino?.nombre_completo)}
                    className="p-2 text-gray-400 hover:text-red-500 transition" title="Desmarcar asistencia">
                    <FiX size={16} />
                  </button>
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-xs p-6 shadow-2xl mx-4">
            ...
            <div className="flex flex-col gap-3">
              <button onClick={() => descargar('xlsx')}
                ...
                <FiGrid size={22} />
                <div className="text-left"><p className="text-sm font-semibold">Excel (.xlsx)</p><p className="text-xs text-green-600">Hoja de cálculo editable</p></div>
              </button>
